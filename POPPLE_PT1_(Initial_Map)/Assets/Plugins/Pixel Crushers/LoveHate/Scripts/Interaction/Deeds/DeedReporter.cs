    ...
    public class DeedReporter : MonoBehaviour
    {
        ...
        public virtual void ReportDeedByActor(FactionMember actor, string tag, FactionMember target)
        {
            ...
            if (FindDeedTemplate(tag, out deedTemplate))
            {
                ...
                var minAffinityEffect = deedTemplate.minAffinityEffect;
                var maxAffinityEffect = deedTemplate.maxAffinityEffect;
                // Old templates may have zero for min/max effect, in which case use [-100,100].
                if (minAffinityEffect == 0 && maxAffinityEffect == 0) 
                {
                    minAffinityEffect = -100;
                    maxAffinityEffect = 100;
                }
                var deed = Deed.GetNew(deedTemplate.tag, actor.factionID, target.factionID, deedTemplate.impact,
                                       ...
                                       deedTemplate.permittedEvaluators,
                                       minAffinityEffect, maxAffinityEffect, deedTemplate.noRepeatDuration);
