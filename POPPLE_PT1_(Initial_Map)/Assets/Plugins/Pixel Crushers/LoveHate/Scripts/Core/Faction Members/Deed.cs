...
namespace PixelCrushers.LoveHate
{
    ...
    public class Deed
    {
        ...
        /// </summary>
        [Range(-100, 100)]
        public float minAffinityEffect = -100;

        /// <summary>
        /// Evaluating this deed will never cause faction's affinity to go below above value.
        /// </summary>
        [Range(-100, 100)]
        public float maxAffinityEffect = -100;

        /// <summary>
        /// Faction members will ignore deeds that are repeated within this duration.
        /// </summary>
        public float noRepeatDuration = 0;

        /// <summary>
        /// A deed object can have multiple references to it. When there are no longer any
        /// references to a deed object, we can return it to the pool.
        /// </summary>
        public int refCount;
        ...
        public void Assign(string tag, int actorFactionID, int targetFactionID, float impact,
                           ...
                           float minAffinityEffect = -100, float maxAffinityEffect = 100,
                           float noRepeatDuration = 0)
        {
            ...
            this.minAffinityEffect = minAffinityEffect;
            this.maxAffinityEffect = maxAffinityEffect;
            this.noRepeatDuration = noRepeatDuration;
            this.refCount = 1;
        ...
        /// <param name="actorPowerLevel">Actor power level.</param>
                                  ...
                                  float noRepeatDuration = 0)
        {
            ...
            deed.Assign(tag, actorFactionID, targetFactionID, impact, aggression, actorPowerLevel, traits, 
                permittedEvaluators, minAffinityEffect, maxAffinityEffect, noRepeatDuration);
            return deed;
        }
        ...
        /// </summary>
        /// <returns>The new.</returns>
        /// <param name="deedTemplate">Deed template (typically from DeedTemplateLibrary).</param>
        /// <param name="actorFactionID">Actor faction ID.</param>
        /// <param name="targetFactionID">Target faction ID.</param>
        /// <param name="actorPowerLevel">Actor power level.</param>
        /// <param name="permittedEvaluators">Who is allowed to evaluate the deed.</param>
        public static Deed GetNew(DeedTemplate deedTemplate, int actorFactionID, int targetFactionID, float actorPowerLevel, 
                                  PermittedEvaluators permittedEvaluators = PermittedEvaluators.Everyone)
        {
            var deed = pool.Get();
            deed.Assign(deedTemplate.tag, actorFactionID, targetFactionID, deedTemplate.impact, deedTemplate.aggression, 
                actorPowerLevel, deedTemplate.traits, permittedEvaluators);
            deed.category = deedTemplate.category;
            return deed;
        }
